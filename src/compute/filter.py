...
from config.compute import PullBackFilterConfig

logger = logging.getLogger(__name__)


def basic_long_filter(
    data: pl.LazyFrame | pl.DataFrame,
    symbol_list: list[str],
    scan_date: datetime,
) -> pl.DataFrame:
    logger.info("Applying Basic Long Filter")
    ...
    res = (
        ...
        .filter(
            ...
            & (
                ...
                | (
                    pl.col("close_sma_200").is_between(
                        lower_bound=pl.min_horizontal(
                            "close_ema_9", "close_ema_21", "close_sma_50"
                        ),
                        upper_bound=pl.max_horizontal(
                            "close_ema_9", "close_ema_21", "close_sma_50"
                        ),
                        closed="both",
                    )
    ...
    logger.info(f"Symbols after basic long filter: {len(res)}")
    return res
...
def basic_short_filter(
    ...
    logger.info("Applying Basic Short Filter")
    logger.info(f"Number of stocks in symbol list: {len(symbol_list)}")
