...
from config.compute import MarketBreadthConfig
...
def compute_breadth(data: pl.DataFrame) -> pl.LazyFrame:
    ...
    filtered = (
        ...
        .filter(
            (pl.col("close") >= MarketBreadthConfig.MIN_PRICE)
            & (pl.col("_avg_vol_20") >= MarketBreadthConfig.MIN_AVG_VOL)
        )
        .drop("_avg_vol_20")
    )
    ...
    symbol_df = (
        filtered.with_columns(
            [
                ...
                .over(partition_by="symbol", order_by="timestamp")
                .alias(f"sma_{n}")
                for n in MarketBreadthConfig.SMA_DAYS
            ]
            + [
                # Rolling High / Low
                pl.col("close")
                ...
                .over(partition_by="symbol", order_by="timestamp")
                .alias(f"high_{n}")
                for n in MarketBreadthConfig.HL_DAYS
            ]
            + [
                pl.col("close")
    ...
    market_df = (
        ...
        .agg(
            ...
            + [
                pl.col(f"nh_{n}").sum().alias(f"nh_{n}")
                for n in MarketBreadthConfig.HL_DAYS
            ]
            + [
                pl.col(f"nl_{n}").sum().alias(f"nl_{n}")
                for n in MarketBreadthConfig.HL_DAYS
            ]
            + [
                pl.col(f"nh_{n}").mean().round(4).alias(f"pct_nh_{n}")
                for n in MarketBreadthConfig.HL_DAYS
            ]
            + [
                pl.col(f"nl_{n}").mean().round(4).alias(f"pct_nl_{n}")
                for n in MarketBreadthConfig.HL_DAYS
            ]
            + [
    ...
    derived_df = (
        ...
        .with_columns(
            pl.col("net_adv")
            .ewm_mean(span=MarketBreadthConfig.MCO_SPANS[0])
            .alias("_mco_ema19"),
            pl.col("net_adv")
            .ewm_mean(span=MarketBreadthConfig.MCO_SPANS[1])
            .alias("_mco_ema39"),
            (
                pl.col("nh_252").cast(pl.Float64)
                / (pl.col("nh_252") + pl.col("nl_252") + 1).cast(pl.Float64)
            )
            .ewm_mean(span=MarketBreadthConfig.HL_INDEX_SPAN)
            .round(4)
            .alias("hl_index_252"),
            (
    ...
    percentile_df = derived_df.with_columns(
        rolling_percentile(
            pl.col("avg_proximity_sma_20"), MarketBreadthConfig.MICRO_WINDOW
        ).alias("p_trend"),
        rolling_percentile(
            pl.col("pct_above_sma_20"), MarketBreadthConfig.MICRO_WINDOW
        ).alias("p_trend_binary"),
        rolling_percentile(pl.col("pct_nh_20"), MarketBreadthConfig.MICRO_WINDOW).alias(
            "p_momentum"
        ),
        rolling_percentile(
            pl.col("nh_20").cast(pl.Float64) / (pl.col("nl_20") + 1).cast(pl.Float64),
            MarketBreadthConfig.MICRO_WINDOW,
        ).alias("p_expansion"),
        rolling_percentile(
    ...
    scored_df = (
        ...
        .with_columns(
            pl.col("breadth_raw")
            .ewm_mean(span=MarketBreadthConfig.EWM_SPANS["fast"])
            .alias("_ema_fast"),
            pl.col("breadth_raw")
            .ewm_mean(span=MarketBreadthConfig.EWM_SPANS["medium"])
            .alias("_ema_medium"),
            pl.col("breadth_raw")
            .ewm_mean(span=MarketBreadthConfig.EWM_SPANS["slow"])
            .alias("_ema_slow"),
